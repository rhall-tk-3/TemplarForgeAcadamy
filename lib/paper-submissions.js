...
// Build ALLOWED_PROGRAMS dynamically from the curriculum index so it never
// goes stale when new programs are added. Falls back to an empty Set (which
// would still block uploads) rather than crashing the server on a bad read.
function _loadAllowedPrograms() {
  try {
    const indexPath = path.resolve("src/config/curriculum/index.json");
    const index = JSON.parse(fs.readFileSync(indexPath, "utf8"));
    const slugs = (index.programs || []).map(p => p.slug).filter(Boolean);
    return new Set(slugs);
  } catch (_) {
    // Fallback: hard-coded list of all current program slugs
    return new Set([
      "squire", "levie", "corporal", "sergeant", "sfc",
      "knight-aspirant", "knight", "lieutenant",
      "captain", "major", "commander", "chaplain"
    ]);
  }
}
const ALLOWED_PROGRAMS = _loadAllowedPrograms();
...
function readMetadata() {
  ensureStore();
  try {
    const mtime = fs.statSync(META_FILE).mtimeMs;
    if (_metaCache && mtime === _metaCacheMtime) return _metaCache;
    _metaCache      = JSON.parse(fs.readFileSync(META_FILE, "utf8"));
    _metaCacheMtime = mtime;
    return _metaCache;
  } catch (_e) {
    _metaCache     = JSON.parse(fs.readFileSync(META_FILE, "utf8"));
    _metaCacheMtime = 0;
...
function writeMetadata(data) {
  ...
  _metaCache      = data;
  _metaCacheMtime = fs.statSync(META_FILE).mtimeMs;
}
